let isMouseDragging = false;  // Renamed variable
let lastMouseX = 0;
let lastMouseY = 0;
let rotationX = 0;
let rotationY = 0;
...
function mousePressed() {
  isMouseDragging = true;  // Use renamed variable
  lastMouseX = mouseX;
  lastMouseY = mouseY;
}

function mouseReleased() {
  isMouseDragging = false;  // Use renamed variable
}
...
function mouseDragged() {
  ...
  lastMouseX = mouseX;
  lastMouseY = mouseY;
  if (params.camera === 'none' && isMouseDragging) {  // Use renamed variable
    rotationX += dx * params.rotationSpeed;
    rotationY += dy * params.rotationSpeed;
  }
}
...
let gui;
let randomAxis;
let messages = ['We are lost souls', 'Are we lost?', 'Can we save ourselves?'];
let messageIndex = 0;
let charIndex = 0;
...
function draw() {
  ...
  if (params.camera === 'none' && mouseDragged) {
    rotateX(rotationX);
    rotateY(rotationY);
  }
...
function updateTitleTypewriter() {
  let titleContentElement = document.querySelector('#title-content');
  let currentMessage = messages[messageIndex];

  // Add or remove characters
  titleContentElement.innerText = currentMessage.slice(0, charIndex);

  // Update character index and possibly the message index
  ...
  // Schedule next update. Adjust the delay values as needed.
  let delay = typingDirection === 1 ? 200 : 100;  // Slower when typing, faster when deleting
  if (charIndex === 0) delay = 1000;  // Wait before typing new message
  setTimeout(updateTitleTypewriter, delay);
}
