(function (global, factory) {
  ...
  var isValidString = function isValidString(argsText) {
    if (
      argsText &&
      argsText !== "" &&
      (typeof argsText === "string" || argsText instanceof String)
    ) {
      return true;
    }
    return false;
  ...
  var setArgs = function setArgs(args, t) {
    ...
    if (args && passedAsObject) {
      obj.target =
        typeof args.target !== "undefined" ? args.target : "[data-scrambler]";
      obj.random =
        typeof args.random !== "undefined" ? args.random : [1000, 3000];
      obj.speed = typeof args.speed !== "undefined" ? args.speed : 100;
      obj.text = typeof args.text !== "undefined" ? args.text : false;
      obj.beforeEach =
        typeof args.beforeEach !== "undefined" && isFunction(args.beforeEach)
          ? args.beforeEach
          : false;
      obj.afterEach =
        typeof args.afterEach !== "undefined" && isFunction(args.afterEach)
          ? args.afterEach
          : false;
      obj.beforeAll =
        typeof args.beforeAll !== "undefined" && isFunction(args.beforeAll)
          ? args.beforeAll
          : false;
      obj.afterAll =
        typeof args.afterAll !== "undefined" && isFunction(args.afterAll)
          ? args.afterAll
          : false;
      obj.errorHandler =
        typeof args.errorHandler !== "undefined" &&
        isFunction(args.errorHandler)
          ? args.errorHandler
          : false;
    }
    return obj;
  ...
  var killCheck = function killCheck(newLetters, truth) {
    var result =
      newLetters.length === truth.length &&
      newLetters.every(function (e, i) {
        return e === truth[i];
      });
    if (result) {
      return true;
  ...
  var defineEndText = function defineEndText(end) {
    if (
      ...
      !(typeof end === "string" || end instanceof String)
    )
      return false; // break if no string passed
    var endText = end;
    ...
    return new Promise(function (resolve, reject) {
      ...
      if (element.getAttribute("data-scramble-active") !== "true") {
        ...
        if (isValidString(scrambleFireArgs.text)) {
          ...
          element.getAttribute("data-scramble-text") &&
          element.getAttribute("data-scramble-text") !== ""
        ) {
          newTextResult = defineEndText(
            element.getAttribute("data-scramble-text")
          );
        }
        // reset vars
          ...
          if (arr && isArray(arr)) {
            var _loop = function _loop(i) {
              setTimeout(function () {
                ...
                isObject(scrambleFireArgs),
                scrambleFireArgs.random,
  ...
  function ScramblerSetup(passedArgs) {
    if (isValidArgType(passedArgs)) {
      ...
      var scrambleElements = isObject(scrambleFireArgs)
        ? _toConsumableArray(document.querySelectorAll(scrambleFireArgs.target))
        : _toConsumableArray(document.querySelectorAll(scrambleFireArgs));
